...
import {AAHelper} from "test/paymasterV3EPv9/fork/helpers/AAHelper.t.sol";
...
contract PaymasterHelper is AAHelper {
    ...
    {
        bytes32 rawHash = paymaster.getHash(_mode, _userOp);
        bytes32 ethSignedHash = MessageHashUtils.toEthSignedMessageHash(rawHash);
        uint256 PK = __PAYMASTER_SIGNER_PRIVATE_KEY;
        ...
        returns (bytes memory paymasterData)
    {
        ...
        if (_mode == VERIFYING_MODE) {
            ...
            if ((_combinedByte & 0x02) != 0) {
                // recipientPresent
                paymasterData = abi.encodePacked(paymasterData, address(__OWNER_7702_ADDRESS)); // recipient (20 bytes)
            }
        }
    }
    ...
    function _depositToEP() internal {
        vm.startPrank(__PAYMASTER_ADMIN_ADDRESS);
        paymaster.deposit{value: 1 ether}();
        paymaster.addStake{value: 1 ether}(860);
        vm.stopPrank();
    }
}
